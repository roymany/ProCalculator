...
def test_complex_expressions():
    ...
    assert all_together("2.5* 4+ (3  !) - 2*2*(1@1)") == "12.0"
    assert all_together("((4 !#-  -3)$  4-2)* (100 & (3%6) +4)") == "49.0"
    assert all_together("(4!#--3$4-2)!#!# ^ (100@5)") == "1.0"
    assert all_together("5!/4+9^0.5-3*2^2+33#") == "27.0"
    assert all_together("(((3+2*4)/2)#) ^ (2.5 + 0.5)") == "1000.0"
    assert all_together("5!@4.2*3&1+(2^3)%8") == "62.1"
    assert all_together("(5 !& 4+9^ 2) * 2^ 2") == "340.0"
